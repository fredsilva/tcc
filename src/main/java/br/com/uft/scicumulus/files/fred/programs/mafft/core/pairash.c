...
static float callrash( int mem1, int mem2, char **mseq1, char **mseq2, double *equiv, char *fname1, char *chain1, char *fname2, char *chain2, int alloclen )
{
	...
	sprintf( dirname, "_rashtmp_%d-%d", mem1, mem2 );
	sprintf( com, "rm -rf %s; mkdir %s", dirname, dirname );
	system( com );
...
#endif
#if 1  // 091127, pdp nai!
		sprintf( com, "cd %s; %s/PDP_ASH.pl --qf ../%s.pdb --qc %s --tf ../%s.pdb --tc %s > _rashout 2>_dum", dirname, whereispairalign, fname1, chain1, fname2, chain2 );
#else
		sprintf( com, "cd %s; %s/rash --qf ../%s.pdb --qc %s --tf ../%s.pdb --tc %s > _rashout 2>_dum", dirname, whereispairalign, fname1, chain1, fname2, chain2 );
#endif
		fprintf( stderr, "command = %s\n", com );
		res = system( com );
		if( res )
		{
			fprintf( stderr, "Error in PDP_ASH\n" );
			exit( 1 );
		}
	...
	else
	{
		...
		sprintf( com, "grep -v successful %s > %s/_rashout", dirname, cachefile );
		system( com );
	}
	...
	if( usecache && runnow )
	{
		...
		system( com );
		sprintf( com, "cat _rashout >>  %s", cachefile );
		system( com );
	}

	sprintf( com, "%s/_rashout", dirname );
	fp = fopen( com, "r" );
	if( !fp )
	{
		fprintf( stderr, "Cannot open _rashout\n" );
		exit( 1 );
	}
...
//	for( i=0; i<njob; i++ )
//		fprintf( stdout, ">%d_%s-%s\n%s\n", i+1, strfiles[i], chainids[i], checkseq[i] );
