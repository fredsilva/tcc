...
void profilealignment( int n0, int n1, int n2, char **aln0, char **aln1, char **aln2, int alloclen, char alg ) // n1 ha allgap
{
	...
	if( alg == 'M' )
		MSalignmm( aln0, aln2, effarr0, effarr2, n0, n2, alloclen, NULL, NULL, NULL, NULL );
	else
		A__align( aln0, aln2, effarr0, effarr2, n0, n2, alloclen, NULL, &dumfl, NULL, NULL, NULL, NULL );
...
//		fprintf( stderr, "seq[%d] = %s\n", i, seq[i] );
	...
	*iptr = -1;
	...
	while( *tmpgaplen != -1 ) *gaplen++ = *tmpgaplen++;
